...
# پیاده‌سازی بلوم فیلتر

## معرفی
این مخزن شامل پیاده‌سازی یک بلوم فیلتر ساده در زبان پایتون است. بلوم فیلتر یک ساختار داده‌ای احتمالی است که برای بررسی عضویت یک عنصر در یک مجموعه استفاده می‌شود. این ساختار حافظه‌ای کارآمد است و از نظر سرعت نیز عملکرد بالایی دارد، اما ممکن است نتایج مثبت کاذب (False Positive) بدهد، در حالی که نتایج منفی کاذب (False Negative) ندارد.

## نحوه عملکرد
1. **ابتدایی‌سازی**: بلوم فیلتر با موارد زیر مقداردهی اولیه می‌شود:
   - یک آرایه بیتی با اندازه ثابت.
   - تعداد مشخصی از توابع هش.

2. **اضافه کردن یک عنصر**:
   - هر عنصر چندین بار با استفاده از توابع هش مستقل هش می‌شود.
   - مقادیر هش به‌دست‌آمده به عنوان شاخص‌هایی استفاده می‌شوند تا بیت‌های متناظر در آرایه به 1 تغییر یابند.

3. **بررسی عضویت**:
   - عنصر دوباره هش شده و بیت‌های متناظر در آرایه بررسی می‌شوند.
   - اگر تمام بیت‌ها 1 باشند، عنصر احتمالاً در مجموعه است. در غیر این صورت، قطعاً نیست.

## ویژگی‌های کلیدی
- **کارایی حافظه**: از یک آرایه بیتی با اندازه ثابت استفاده می‌کند.
- **سرعت**: بررسی عضویت و اضافه کردن عناصر در زمان ثابت انجام می‌شود.
- **ماهیت احتمالی**: ممکن است نتایج مثبت کاذب بدهد، اما نتایج منفی کاذب ندارد.

## ساختار کد
- **کلاس `BloomFilter`**:
  - `__init__(size, hash_count)`: آرایه بیت را مقداردهی اولیه می‌کند و تعداد توابع هش را تنظیم می‌کند.
  - `_hashes(item)`: مقادیر هش را برای یک عنصر تولید می‌کند.
  - `add(item)`: یک عنصر را به بلوم فیلتر اضافه می‌کند.
  - `check(item)`: بررسی می‌کند که آیا یک عنصر احتمالاً در مجموعه وجود دارد یا خیر.

- **اسکریپت اصلی**:
  - استفاده از بلوم فیلتر را با اضافه کردن عناصر و بررسی عضویت نمایش می‌دهد.
...
print(bf.check("apple"))  # خروجی: True
print(bf.check("banana")) # خروجی: True
print(bf.check("grape"))  # خروجی: False
```

## محدودیت‌ها
- **نتایج مثبت کاذب**: با اضافه شدن تعداد بیشتری از عناصر، احتمال نتایج مثبت کاذب افزایش می‌یابد.
- **اندازه ثابت**: اندازه آرایه بیت و تعداد توابع هش باید بر اساس اندازه ورودی مورد انتظار و نرخ نتایج مثبت کاذب انتخاب شوند.

## کاربردها
- بهینه‌سازی درخواست‌ها در کش و پایگاه داده.
- فیلتر کردن هرزنامه‌ها.
- تحلیل شبکه.

## پیش‌نیازها
- پایتون 3.x
- کتابخانه استاندارد: `hashlib`
